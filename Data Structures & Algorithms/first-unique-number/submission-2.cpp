...
private:
    // Alternative:
    // Can use a hash map instead of two separeted set
    // use the "status" value (not_seen, unique, non_unique) for each num
...
public:
    FirstUnique(vector<int>& nums) {
        ...
        for (auto num: nums) {
            this->add(num); // use API to avoid duplicate code
