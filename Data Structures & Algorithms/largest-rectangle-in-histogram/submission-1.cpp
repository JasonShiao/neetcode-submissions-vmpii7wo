    ...
    int largestRectangleArea(vector<int>& heights) {
        ...
        for (int i = 0; i < heights.size(); i++) {
            ...
            if (heights[i] > height_stack.top().first) {
                ...
                int last_popped_idx = 0; // NOTICE: this is critical
                while (!height_stack.empty() && heights[i] < height_stack.top().first) {
                    ...
                    last_popped_idx = height_stack.top().second; // NOTICE: this is critical
                    height_stack.pop();
                }
                height_stack.push({heights[i], last_popped_idx}); // NOTICE: this is critical
            }
        }
