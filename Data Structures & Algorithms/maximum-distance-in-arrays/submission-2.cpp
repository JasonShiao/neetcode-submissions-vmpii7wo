...
public:
    int maxDistance(vector<vector<int>>& arrays) {
        ...
        for (int i = 2; i < arrays.size(); i++) {
            if (arrays[i][arrays[i].size() - 1] >= arrays[max_idx][arrays[max_idx].size() - 1]) {
                max_2nd_idx = max_idx;
                max_idx = i;
            } else if (arrays[i][arrays[i].size() - 1] >= arrays[max_2nd_idx][arrays[max_2nd_idx].size() - 1]) {
                max_2nd_idx = i;
            }
        ...
        if (max_idx != min_idx) {
            return arrays[max_idx][arrays[max_idx].size() - 1] - arrays[min_idx][0];
        } else {
            return max(arrays[max_2nd_idx][arrays[max_2nd_idx].size() - 1] - arrays[min_idx][0],
                    arrays[max_idx][arrays[max_idx].size() - 1] - arrays[min_2nd_idx][0]);
        }
