...
public:
    int subarraySum(vector<int>& nums, int k) {
        ...
        int res = 0;
        for (auto& num: nums) {
            // count the number of valid subarray "ends exactly at num"
            // i.e. = how many ending pos s.t. prefix sum = (accum_sum - k)

            accum_sum += num; // accumulated sum until num (follow index order)
